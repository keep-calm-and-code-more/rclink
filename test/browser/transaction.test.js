...
import { CreateKeypair, GetKeyPEM } from "../../lib/crypto";
...
describe("测试交易生成功能", () => {
    ...
    it("使用某私钥生成的签名交易信息，使用相应的公钥验证应能通过", () => {
        ...
        const isValid = t.verifySignedTransaction(kp1.pubKeyObj, "ecdsa-with-SHA1");
        expect(isValid).toBeTruthy();
    });
    it("使用某私钥生成的签名交易信息，使用非对应公钥验证不应通过", () => {
        const t = new Transaction({
            type: 2,
            pubKeyPEM: GetKeyPEM(kp1.pubKeyObj),
            name: "nnnnn",
        ...
        const isValid = t.verifySignedTransaction(kp2.pubKeyObj, "ecdsa-with-SHA1");
        expect(isValid).toBeFalsy();
    });
    it("使用某加密pem格式私钥，输入正确密码生成的签名交易信息，使用相应公钥验证应能通过", () => {
        const t = new Transaction({
            type: 2, pubKeyPEM: GetKeyPEM(kp1.pubKeyObj), name: "xxxxx", function: "function", args: ["123", "456"],
        });
    ...
    it("使用某加密pem格式私钥生成签名交易信息，输入密码错误时，应该抛出异常", () => {
        const t = new Transaction({
            type: 2, pubKeyPEM: GetKeyPEM(kp1.pubKeyObj), name: "yyyyy", function: "function", args: ["123", "456"],
        });
        expect(() => {
