...
describe('测试交易生成功能', () => {
    ...
    beforeAll(async () => {
        ...
        tSignedBuffer = await t.createSignedTransaction(kp1.prvKeyObj, "SHA1")
    }) 

    test('使用某私钥生成的签名交易信息，使用相应的公钥验证应能通过', async () => {
        let isValid = await t.verifySignedTransaction(tSignedBuffer, kp1.pubKeyObj, "SHA1")
        expect(isValid).toBeTruthy()
    })
    test('使用某私钥生成的签名交易信息，使用非对应公钥验证不应通过', async () => {
        let isValid = await t.verifySignedTransaction(tSignedBuffer, kp2.pubKeyObj, "SHA1")
